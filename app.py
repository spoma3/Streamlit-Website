...
with st.container():
    ...
    with left_column:
        ...
        st.caption("Figure 1. As shown in the plot, the R² error levels out as the number of trees passes 460, indicating that increasing the number of trees past 460 doesn’t significantly improve the model's performance. To prevent overfitting the model had its training and testing split set to 20 and 80 respectively to prevent the model from memorizing the data.")
        ...
        st.caption("Figure 2. This plot demonstrates that the model steadily decreased its loss over the 4 epochs, which indicates that the training process was effective and the model was able to learn meaningful patterns in the data.")
        st.title("Bayesian Neural Network")
        st.title("Y_train quantile")
        st.image("y_train_dist.png")
        st.caption("Figure 3. This image shows the smoothing effect of the quantile distribution function which was implemented to give y_train and y_test a more gaussian curve to make it easier for the bayesian neural network to process and learn from the data.")
        st.title("Bayesian Neural Network Model")
        st.image("BNN Loss.png")
        st.caption("Figure 4. The loss of the bayesian neural network model to the training data is shown here. A key thing to note is that the loss trends downwards over time except for the last 10 epochs where it trends up highlighting that the model’s overfitting protections work well")
        st.image("BNN Uncertainty.png")
        st.caption("Figure 5. Here is the uncertainty as you can see it seems to randomly fluctuate and never significantly change in the model from what we found.")
